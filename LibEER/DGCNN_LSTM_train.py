...
# DGCNN + BiLSTM + Attention — temporal extension of the DGCNN spatial model.
#
# Data format: (B, T, V, F)  — requires sample_length > 1 to form the time sequence.
# Recommended settings mirror ST-GCLSTM: 10s windows with 50% overlap →
#   -time_window 1  -sample_length 10  -stride 5
#
# Example commands:
#
# SEED sub-dependent (front-back split, sessions 1&2, replicates DGCNN baseline setup):
#   python DGCNN_LSTM_train.py -onehot -batch_size 16 -lr 0.0015 -sessions 1 2 \
#       -epochs 80 -setting seed_sub_dependent_front_back_setting \
#       -sample_length 10 -stride 5
#
# SEED sub-dependent (train/val/test split):
#   python DGCNN_LSTM_train.py -metrics acc macro-f1 -metric_choose macro-f1 \
...
def main(args):
    ...
    for rridx, (data_i, label_i) in enumerate(zip(data, label), 1):
        ...
        for ridx, (train_indexes, test_indexes, val_indexes) in enumerate(zip(tts['train'], tts['test'], tts['val']), 1):
            ...
            # Same L2 regularisation as DGCNN_train.py
            loss_func  = NewSparseL2Regularization(0.01).to(device)
            output_dir = make_output_dir(args, "DGCNN_LSTM")
